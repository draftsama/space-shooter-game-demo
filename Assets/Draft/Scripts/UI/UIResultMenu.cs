...
public class UIResultMenu : Singleton<UIResultMenu>
{
        ...
        _Disposable = m_CanvasGroup.LerpAlpha(500, GlobalConstant.ALPHA_VALUE_VISIBLE,_useUnscaleTime: true,
            () =>
            {
                _Disposable = m_LineTransform.LerpWidth(_LINE_WIDTH_TARGET, 500, Easing.Ease.EaseInOutQuad, true,
                    () =>
                    {
                        _Disposable = m_TextTransform.LerpAnchorPosition(Vector2.up * _TEXT_POS_Y_TARGET, 800, _useUnscaleTime: true,_completed:
                                () =>
                                {
                                    _Disposable =   UpdateTextValue(5000, m_ScoreText, 0f, (float)_score)
                                        .Subscribe(_ =>
                                        {
                                            _Disposable = Observable.Timer(TimeSpan.FromMilliseconds(3000)).Subscribe(_=> _oncompleted?.Invoke()).AddTo(this);
                                        }).AddTo(this);
                                }).AddTo(this);
                    }).AddTo(this);
    ...
    private IObservable<Unit> UpdateTextValue(int _milliseccond,TextMeshProUGUI _textMeshPro, float _startValue,float _targetValue)
    {
